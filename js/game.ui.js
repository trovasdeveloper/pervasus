...
function renderScene(sceneId) {
  ...
  if (!scene) {
    ...
    if (narrativeOutput)
      narrativeOutput.innerHTML = "<h1>DATA FRAGMENT MISSING</h1>";
    return;
  }
  ...
  if (controlContainer) {
    switch (scene.inputType) {
      ...
      case INPUT_TYPE_TEXT_INPUT:
        // Check if the player is currently in the 30-minute "lock-out"
        const lockoutTimestamp = localStorage.getItem("joker_lockout");
        const now = Date.now();

        if (lockoutTimestamp && now < parseInt(lockoutTimestamp)) {
          const remainingMs = parseInt(lockoutTimestamp) - now;
          const remainingMins = Math.ceil(remainingMs / 60000);
          // Kinda scuffed to have this hardcoded here but whatever
          controlContainer.innerHTML = `
            <div class="text-center p-3" style="border: 2px solid red; background: rgba(255,0,0,0.1);">
                <p class="text-danger"><strong>JOKER IS NAPPING</strong></p>
                <p>He's too fried to talk. Come back in <strong>${remainingMins} minutes</strong>.</p>
            </div>`;
        } else {
          controlContainer.innerHTML = `
            <div class="input-group mb-3">
                <input type="text" id="joker-guess-input" class="form-control" placeholder="Who's honest?">
                <button class="control-button" type="button" onclick="submitJokerGuess('${sceneId}')">TELL HIM</button>
            </div>`;
        }
        break;
...
// Joker Quest Input Handler
function submitJokerGuess(sceneId) {
  const inputField = document.getElementById("joker-guess-input");
  if (!inputField) return;

  const guess = inputField.value.trim().toLowerCase();
  const scene = gameData[sceneId];

  if (guess === scene.correctName.toLowerCase()) {
    localStorage.removeItem("joker_lockout");
    continueNarrative(scene.nextSceneId);
  } else {
    // Set the 30-minute lockout
    const expiry = Date.now() + 30 * 60 * 1000;
    localStorage.setItem("joker_lockout", expiry);

    // Use the engine to load the failure scene
    continueNarrative("node_stoner_joker_wrong");
  }
}
